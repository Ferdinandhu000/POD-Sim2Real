#!/usr/bin/env bash
set -e
# Run all experiments in yaml_main_v6 or pass custom arguments
# Usage:
#   # Run all v6 configs sequentially on GPU 0:
...
if [ "$#" -eq 0 ]; then
    python -m pod_sim2real.training.train --config-dir yaml_main_v6/
else
